    ...
    <div>
      <div class="bg-slate-800 border-slate-400 rounded-md p-8 shadow-lg backdrop-filter backdrop-blur-sm bg-opacity-30 relative">
        ...
        <div class="relative my-5">
          <input v-model="data.username" type="email" class="block w-72 py-2 px-0 text-sm text-color-white bg-transparent border-0 border-b-2
        ...
        <button @click="login" class="w-full mb-4 text-lg mt-6 rounded-full bg-color-white text-emerald-800 hover:bg-emerald-600 hover:text-color-white py-2 transition-colors duration-300">Login</button>
      </div>
    </div>
  </div>
</template>
